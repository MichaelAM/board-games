...
function sendJSONData(sendScript:string, jsonString:string, success:Function, failure:Function, restMethod:string = "POST"):void {
    // send out AJAX request
    let xmlhttp:XMLHttpRequest = new XMLHttpRequest();
    xmlhttp.addEventListener("load", (e:Event) => {
        // has the response been received successfully?
    ...
    xmlhttp.open(restMethod, sendScript, true);
